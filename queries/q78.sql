...
SELECT
  ...
  ROUND(ss_qty / NULLIF(COALESCE(ws_qty, 0) + COALESCE(cs_qty, 0), 0), 2) AS ratio,
...
ORDER BY ss_sold_year, ss_item_sk, ss_customer_sk, ss_qty DESC,
  ss_wc DESC, ss_sp DESC, other_chan_qty, other_chan_wholesale_cost, other_chan_sales_price, ratio
LIMIT 100