...
wr AS (
  SELECT wr_web_page_sk AS ws_web_page_sk,
...
FROM (
  SELECT 'store channel' AS channel, ss.s_store_sk AS id, sales,
    COALESCE(returns_amt, 0.00) AS returns_amt,
    (profit - COALESCE(profit_loss, 0.00)) AS profit
  ...
  SELECT 'web channel' AS channel, ws.ws_web_page_sk AS id, sales,
    COALESCE(returns_amt, 0.00) AS returns_amt,
    (profit - COALESCE(profit_loss, 0.00)) AS profit
  FROM ws LEFT JOIN wr ON ws.ws_web_page_sk = wr.ws_web_page_sk
) x
